...
```{r}
pb <- readRDS("pb.rds")
pb <- SEtools::log2FC(pb, fromAssay = "sct", controls = pb$condition=="CTRL", by=pb$cluster, isLog = TRUE)
...
pb <- SEtools::log2FC(pb, fromAssay = "logcpm", controls = pb$condition=="CTRL", by=pb$cluster)
...
write.table(cbind(gene=row.names(dea2), dea2[,-ncol(dea2)]), "contributions_bytop.tab", row.names=FALSE, col.names=TRUE, sep="\t", quote=TRUE)
